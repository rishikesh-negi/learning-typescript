...
export type LoggerCallback = (s1: string, s2: string) => string;
...
// Creating a type alias and letting TS infer the return type for functions conforming to that type alias:
export type SupportResponse = (name: string) => string;

// These function conform to the SupportResponse type and allow TS to infers their return type:
...
// Type aliases are scoped only to TS and cannot be referenced for JS oeprations like variable comparison. The SupportResponse type alias, for example, cannot be used in program logic in JS.
