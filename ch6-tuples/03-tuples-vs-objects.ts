...
// On the other hand, while consuming a function that returns an object, we can desctructure only the properties that we need. If we want to consume a function that returns a tuple or array, we would have to use commas to skip the values that we don't need. Ex:
...
// Most of the times, objects are the way to go, but there are cases where tuples are more ideal. Based on the above information and the use cases, we can now make a better decision while choosing between the two data structures.
